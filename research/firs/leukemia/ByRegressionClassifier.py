...
from datamining.own.classification import RegressionClassifier
...
def main():
    ...
    while len(cond[cond == True]) < 14:
        value = np.max(w[w < value])
        cond = w >= value
        print(compactness(X_train[:, cond], y_train))
    ...
    print("RegressionClassifier", mean6)
